...
Revision ID: 2719bce12285
Revises: 81e4579915ed
Create Date: 2025-05-28 00:51:47.420589
...
def upgrade() -> None:
    ...
    op.add_column(
        "manmanstatusupdate",
        sa.Column(
            "slack_message_id", sqlmodel.sql.sqltypes.AutoString(), nullable=True
        ),
        schema="fcm",
    )
    op.create_index(
    ...
    op.create_unique_constraint(
        None, "manmanstatusupdate", ["service_id", "service_type"], schema="fcm"
    )
    # ### end Alembic commands ###

    # add new status update seed
    op.execute("""
    INSERT INTO fcm.slackspecialchanneltype (type_name, friendly_type_name)
    VALUES ('manman_status_updates', 'ManMan Status Updates')
    """)


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(None, "manmanstatusupdate", schema="fcm", type_="unique")
    op.drop_index(
        op.f("ix_fcm_manmanstatusupdate_slack_message_id"),
    ...
    # no need to remove the seed data, it will be removed by the drop table command
